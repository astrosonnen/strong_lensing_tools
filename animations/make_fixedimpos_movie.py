...
alpha_dm_xA = alpha_dm(xA, 1., rs, gammadm_0)
alpha_dm_xB = alpha_dm(xB, 1., rs, gammadm_0)
...
for n in range(nframes):
    ...
    gammadm_here = gammadm_0

    # calculates dark matter mass that produces the same image positions
    gnfw_norm_here = (xA - xB - mstar_here*(alpha_star_xA - alpha_star_xB))/(alpha_dm_xA - alpha_dm_xB)
    beta_here = xA - mstar_here*alpha_star_xA - gnfw_norm_here*alpha_dm_xA
    ...
    print(rmuA, rmuB, rmuA/rmuB, gnfw_norm_here)
    ...
    figname = 'fixedimpos_frame_%02d.png'%n
