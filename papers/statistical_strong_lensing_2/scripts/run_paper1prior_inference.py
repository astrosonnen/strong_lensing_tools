...
dt_relerr = mockfile.attrs['dt_relerr']

h0_ref = mockfile.attrs['h']
...
grids_file = h5py.File(griddir+'/%s_lensmodel_caustbetamax_grids.hdf5'%mockname, 'r')
...
gammadm_min = gammadm_grid[0]
gammadm_max = gammadm_grid[-1]
...
gammadm_beta_par = {'name': 'gammadm_beta', 'lower': -1., 'upper': 1., 'guess': 0., 'spread': 0.1}
gammadm_xi_par = {'name': 'gammadm_xi', 'lower': -3., 'upper': 3., 'guess': 0., 'spread': 0.1}
...
h0_par = {'name': 'h0', 'lower': 0.2, 'upper': 1., 'guess': mockfile.attrs['h'], 'spread': 0.01}

mockfile.close()

pars = [lmdm5_mu_par, lmdm5_sig_par, lmdm5_beta_par, lmdm5_xi_par, gammadm_mu_par, gammadm_beta_par, gammadm_xi_par, gammadm_sig_par, laimf_par, h0_par]
npars = len(pars)

# loads the chain from paper 1, to use as a prior
prior_chain = h5py.File('../../statistical_strong_lensing_1/chains/extended_inference.hdf5', 'r')
...
for par in pars:
    parname = par['name']
    if parname in prior_chain:
        prior_list.append(prior_chain[parname][:, prior_burnin:].flatten())
prior_samp = np.array(prior_list)

prior_cov = np.cov(prior_samp)
prior_mu = np.median(prior_samp, axis=1)
...
def logpfunc(p):
    ...
    lmdm5_mu, lmdm5_sig, lmdm5_beta, lmdm5_xi, gammadm_mu, gammadm_beta, gammadm_xi, gammadm_sig, laimf, h0 = p

    sumlogp = multivariate_normal.logpdf(prior_pars, mean=prior_mu, cov=prior_cov) # prior on m200 from weak lensing
    ...
    for i in range(ngal):
        ...
        dt_err_here = dt_relerr * dt_grids[i]*h0_ref/h0
        dt_like = 1./dt_err_here * np.exp(-0.5*(dt_grids[i]*h0_ref/h0 - dt_obs[i])**2/dt_err_here**2)
        ...
        full_integrand = msps_term * msps_like * reff_term * mdm5_term * dt_like * rmur_like * beta_prior * abs(detJ_grids[i]) * gammadm_term
...
output_file = h5py.File('chains/%s_1000lensesprior_inference.hdf5'%mockname, 'w')
