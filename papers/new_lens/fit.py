...
cs_interp = ndinterp.ndInterp(axes, grid_file['cs_grid'][()])

# loads the P_SL normalisation grid
norm_file = h5py.File('pslnorm_grid.hdf5', 'r')
laimf_grid = norm_file['laimf_grid'][()]
psl_norm_grid = norm_file['norm_grid'][()]
psl_norm_spline = splrep(laimf_grid, psl_norm_grid)
...
def logpfunc(p):
    ...
    # obtains the lensing cross-section
    crosssect = cs_interp.eval(np.array((lmvir, lmstar)).reshape((1, 2)))

    # obtains the normalisation of the prior
    prior_norm = splev(laimf, psl_norm_spline)

    return hmf_lprior + shmr_lprior + masssize_lprior + lmsps_like + tein_llike + np.log(crosssect) - np.log(prior_norm), tein
...
output = h5py.File('inference.hdf5', 'w')
