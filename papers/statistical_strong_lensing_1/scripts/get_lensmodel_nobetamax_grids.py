...
grids_file = h5py.File(gridsdir+'/%s_lensmodel_nobetamax_grids.hdf5'%mockname, 'w')
...
for i in range(ngal):
    ...
    for j in range(ngammadm):
        ...
        mu_r_xB = mu_r(xB_samp[i], gnfw_norm_grid[j, :], rs_fixed*np.ones(nlmdm5), gammadm_grid[j], mstar_grid_here, reff) 

        # exclude solutions in which the third image is at xB
        xB_is_third_image = mu_r_xB < 0.
        grid_range[j, xB_is_third_image] = False
