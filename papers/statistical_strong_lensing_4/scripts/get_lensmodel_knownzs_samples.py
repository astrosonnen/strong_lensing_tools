...
samps_file = h5py.File(sampdir+'/%s_lensmodel_knownzs_samps.hdf5'%mockname, 'w')
...
for i in range(nlens):
    ...
    reff = 10.**lreff[i]
    s_cr = splev(zs[i], s_cr_spline)
    mstar_sch = splev(zs[i], mstar_sch_spline)
    ...
    alpha_star_diff = alpha_star(s_cr, xA[i], mstar_min, reff) - alpha_star(s_cr, xB[i], mstar_min, reff)

    lm200_start = 0. * ms_start
    def zerofunc(lm200):
        r200 = (10.**lm200*3./200./(4.*np.pi)/rhoc)**(1./3.) * 1000.
        rs = r200/c200
        nfw_norm = 10.**lm200/nfw.M3d(r200, rs)
        return xA[i] - xB[i] - alpha_star_diff - alpha_dm(s_cr, xA[i], nfw_norm, rs) + alpha_dm(s_cr, xB[i], nfw_norm, rs)
    ...
    for j in range(nwalkers):
        ...
        while not mur_ok and niter < niter_max:
            ...
            alpha_dm_xA = alpha_dm(s_cr, xA[i], nfw_norm_here, rs_here)
            alpha_dm_xB = alpha_dm(s_cr, xB[i], nfw_norm_here, rs_here)
           
            mstar_here = (xA[i] - xB[i] - alpha_dm_xA + alpha_dm_xB)/(alpha_star_xA - alpha_star_xB)

            mur_B_here = mu_r(s_cr, xB[i], nfw_norm_here, rs_here, mstar_here, reff)
    ...
    start = np.array((lm200_start, ms_start)).T
    ...
    def logprior(p):
        ...
        return logphifunc(ms, mstar_sch)
    ...
    group.create_dataset('ms_samp', data=sampler.chain[:, burnin:, 1])
