...
for i in range(nlens):
    ...
    muA_grids.append(group['muA_grid'][()])
    muB_grids.append(group['muB_grid'][()])
    ...
    for j in range(ngamma):
        ...
        def mag_integrand(m):
            ...
            return phifunc(m)/phi_invnorm * (1. - erf((magB_here - maxmagB_det)/2.**0.5/mag_err))
...
ltein_sig_par = {'name': 'ltein_sig', 'lower': 0.1, 'upper': 0.3, 'guess': mockfile.attrs['ltein_sig'], 'spread': 0.01}
...
def logpfunc(p):
    ...
    if npop_good < 0.5*nis:
        return -1e300

    else:
        cs_point = np.array((gamma_is_here, ms_is)).T

        def ltein_integrand(ltein):
            return 1./(2*np.pi)**0.5/ltein_sig * np.exp(-0.5*(ltein - ltein_mu)**2/ltein_sig**2) * (10.**ltein)**2
        ltein_integral = quad(ltein_integrand, -2., 2.)[0]

        cs_is = ltein_integral * cs_interp.eval(cs_point)

        pop_norm = npop_good/cs_is.sum()
        ...
        for i in range(nlens):
            ...
            full_integrand = pop_norm * gamma_term * ltein_term * beta_prior * abs(detJ_grids[i]) * mag_integrals[i]
...
output_file = h5py.File('../chains/%s_lenses_only_impos_only.hdf5'%mockname, 'w')
