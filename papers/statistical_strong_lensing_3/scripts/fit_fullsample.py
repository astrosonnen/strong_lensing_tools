...
for i in range(nlens):
    ...
    for j in range(ngamma):
        ...
        muA_here = group['muA_grid'][j]
        muB_here = group['muB_grid'][j]
        def mag_integrand(m):
            magA_here = m - 2.5*np.log10(abs(muA_here))
            magB_here = m - 2.5*np.log10(abs(muB_here))
            return phifunc(m)/phi_invnorm * np.exp(-0.5*(magA_here - magA_obs[i])**2/mag_err**2) * np.exp(-0.5*(magB_here - magB_obs[i])**2/mag_err**2) * (1. - erf((magB_here - maxmagB_det)/2.**0.5/mag_err)) / splev(m, crosssect_spline) * splev(m, poiss_spline)
...
output_file = h5py.File('../chains/%s_fullsample_inference.hdf5'%mockname, 'w')
